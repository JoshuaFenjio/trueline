                ...
                <div className="card overflow-x-auto !p-0">
                  <table className="w-full min-w-[420px] text-sm">
                    ...
                    <tbody>
                      ...
                      <tr className="border-t" style={{ borderColor: "var(--border)" }}><td className="px-5 py-3 text-ink-muted">Live job ads tracked</td>{cos.map((c) => <td key={c.slug} className="px-5 py-3 text-right tnum">{c.activeN}</td>)}</tr>
                      <tr className="border-t" style={{ borderColor: "var(--border)" }}><td className="px-5 py-3 text-ink-muted">Job ads disclosing pay</td>{cos.map((c) => <td key={c.slug} className="px-5 py-3 text-right tnum" style={{ color: c.disclosurePct >= 50 ? "var(--mint)" : c.disclosurePct < 25 ? "var(--ember)" : undefined }}>{pct(c.disclosurePct)}</td>)}</tr>
